...
function DropDownMenu({ buttonMenu, buttonTop, buttonbottom, text, onClickEdit, onClickExit, classText }) {
    return (
        <div className='relative mt-2'>
            <Menu>
                <Menu.Button className="inline-flex items-center gap-2 focus:outline-none">
                    {buttonMenu}
                    <Text className={`text-gray-500 ${classText}`}>{text}</Text>
                </Menu.Button>

                <div className="relative">
                    <Menu.Items className="absolute top-full left-0 mt-1 w-52 rounded-xl border border-gray-200 bg-white p-1 shadow-lg z-[25] focus:outline-none">
                        <Menu.Item>
                            {({ active }) => (
                                <button
                                    onClick={onClickEdit}
