                ...
                <div className="border rounded-lg p-2" 
                    ...
                    <div className="bg-Gray1 p-2 flex justify-between items-center">
                        <div className="grid gap-2">
                            <Text className={`!font-bold text-lg`}>طلب : {item?.order_code}</Text>
                            <Text className={`text-gray-500 flex items-center gap-1`}>السعر : <span className="text-BgCustom flex gap-1 items-center">{Number(item?.final_price).toLocaleString("ar-EG")} <img src={Rial} alt="" /></span></Text>
                            <Text className={`text-gray-500`}> الحالة : <span className={stateColors[item?.state]}>{stateLabels[item?.state] || "غير معروف"}</span></Text>
                        </div>
                        <div>
                            <svg 
                                width="24" 
